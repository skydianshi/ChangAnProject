#ifndef _CYCLEBUFFER_H_
#define _CYCLEBUFFER_H_
#include "AutoLock.h"
...
public:
	...
	CCycleBuffer() :
			m_pobj(NULL), m_len(0), m_off(0), m_validlen(0), m_iChanged(0) {
	}
	CCycleBuffer(int len) :
			m_pobj(NULL), m_len(len), m_off(0), m_validlen(0) {
	}
	...
	void ReAllocate(int maxlen) {
		...
		if (m_pobj != NULL) {
			int tmp1 =
					((m_validlen + m_off <= m_len) ?
							m_validlen : (m_len - m_off));
	...
	void AddObj(const T* pobj, int objlen) {
		CAutoLock autolock(&m_sec);
		if (m_pobj == NULL) {
...
// 			objlen=m_len;
	...
	void CopyObj(T* pobj, int& objlen) {
		...
		if (m_off == m_len)
			m_off = 0;
		if (tmp1 < getlen) {
			memcpy(pobj + tmp1, m_pobj + m_off, (getlen - tmp1) * sizeof(T));
			m_off += getlen - tmp1;
		}
		objlen = getlen;
		m_off = off;
	...
	int CopySingleObjbyPos(T& obj, int pos) {
		...
		if (pos >= m_validlen)
			return 0;
		int tmp1 =
				(pos + m_off >= m_len) ? (m_off + pos - m_len) : (pos + m_off);
		obj = *(m_pobj + tmp1);
		return 1;
	}
